...
module.exports = {
  ...
  question3: async (req, res, next) => {
    try {
      ...
      const conditionFind = { $expr: { $lte: [d, parseFloat(90000)] } };
    ...
    try {
      let results = await Category.aggregate()
        ...
        .group({
          ...
          totalProduct: {
            $sum: "$products.stock", //query tổng các sản phẩm theo danh mục
            // $sum:1, query theo tổng từng sản phẩm
          },
        })
        .sort({
          totalProduct: -1,
